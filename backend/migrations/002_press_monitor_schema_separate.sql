-- Press Monitor Schema Migration with separate schema
-- Creates tables for Azerbaijan press monitoring system in separate schema

-- Create schema for press monitoring
CREATE SCHEMA IF NOT EXISTS press_monitor;

-- Set search path to include the new schema
SET search_path TO press_monitor, public;

-- Drop existing tables if they exist
DROP TABLE IF EXISTS press_monitor.article_digest_mapping CASCADE;
DROP TABLE IF EXISTS press_monitor.press_digests CASCADE;
DROP TABLE IF EXISTS press_monitor.press_articles CASCADE;
DROP TABLE IF EXISTS press_monitor.language_coverage CASCADE;

-- Table for storing press articles
CREATE TABLE press_monitor.press_articles (
...
CREATE TABLE press_monitor.press_digests (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE press_monitor.article_digest_mapping (
    article_id UUID REFERENCES press_monitor.press_articles(id) ON DELETE CASCADE,
    digest_id UUID REFERENCES press_monitor.press_digests(id) ON DELETE CASCADE,
    PRIMARY KEY (article_id, digest_id)
);

-- Language coverage tracking
CREATE TABLE press_monitor.language_coverage (
...
CREATE INDEX idx_pm_articles_sentiment ON press_monitor.press_articles(sentiment);
CREATE INDEX idx_pm_articles_published ON press_monitor.press_articles(published_date DESC);
CREATE INDEX idx_pm_articles_language ON press_monitor.press_articles(source_language);
CREATE INDEX idx_pm_articles_country ON press_monitor.press_articles(source_country);
CREATE INDEX idx_pm_articles_url ON press_monitor.press_articles(url);
CREATE INDEX idx_pm_language_coverage_checked ON press_monitor.language_coverage(last_checked);

-- Insert initial language data
INSERT INTO press_monitor.language_coverage (language_code, language_name, native_name, region, countries) VALUES
-- Priority: Neighbors
...
-- GRANT ALL ON SCHEMA press_monitor TO your_user;
-- GRANT ALL ON ALL TABLES IN SCHEMA press_monitor TO your_user;